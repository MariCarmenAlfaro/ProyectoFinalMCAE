...
namespace HipicaAlfaro.Api.Controllers
{
    ...
    public class PaymentController : Controller
    {
        ...
        public IActionResult ReadAllWithUserAndPrice()
        {
            ...
            using (var db = new MySqlConnection(_connection))
            {
                var sql = "Select payId, payments.userId, payDate, payMethod, isPaid, prices.price, prices. typeService, userprofile.userName, userprofile.emailAddress from payments inner join prices on payments.priceId =  prices.priceId inner join userprofile on payments.userId= userprofile.userId order by payDate desc;";
